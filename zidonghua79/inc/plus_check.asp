			...
			If Not InStr(UserGroupIDlist,"@7@")>0 Then
				...
				If IsNumeric(Plus_UserEP) Then 
					If CLng(Dvbbs.UserSession.documentElement.selectSingleNode("userinfo/@userep").text)< CLng(Plus_UserEP) Then
						Response.redirect "showerr.asp?ErrCodes=<li>使用"&Plus_Name&"的最少经验是"&Plus_UserEP&",您只有"& Dvbbs.UserSession.documentElement.selectSingleNode("userinfo/@userep").text &"&action=plus"
					End If
				End If
