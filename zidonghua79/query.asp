...
Sub SQLQueryStr()
	...
	Case 3
		'最新50贴
		If Dvbbs.BoardID > 0 then
			SqlColumn = SqlColumn & ",signflag From "&stable&" where BoardID="&trim(request("BoardID"))&" ORDER BY announceID desc"
		Else
...
Sub SearchResult()
	...
	Dim BoardXML
	Set  BoardXML=Application(Dvbbs.CacheName&"_sboardlist").cloneNode(True)
...
Function QueryArrayToxml(DataArray,Recordset,row,xmlroot)
	...
	For i=0 To UBound(DataArray,2)
		...
		For Each rs in Recordset.Fields
			node.attributes.setNamedItem(QueryArrayToxml.createNode(2,LCase(rs.name),"")).text= DataArray(j,i)& ""
			j=j+1
		Next
