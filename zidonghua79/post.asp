...
Class Dvbbs_Post
	...
	Public Function Show_Post_Body()
		...
		mailflag=0
		MyTopicMode=0
		If Dvbbs.UserID = 0 And Dvbbs.GroupSetting(3)=0 Then Dvbbs.AddErrCode(6)
		Dvbbs.ShowErr()
	...
	Public Function Chk_Post()
		...
		If Dvbbs.UserID>0 Then
			If Clng(Dvbbs.GroupSetting(52))>0 And DateDiff("s",Dvbbs.UserSession.documentElement.selectSingleNode("userinfo/@joindate").text,Now)<Clng(Dvbbs.GroupSetting(52))*60 Then Response.redirect "showerr.asp?ErrCodes=<li>"&Replace(template.Strings(21),"{$timelimited}",Dvbbs.GroupSetting(52))&"&action=OtherErr"
