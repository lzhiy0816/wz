...
<tr>
<td width=50% class=forumrowHighlight>登录次数:<input type=radio value=more name="loginR" checked>&nbsp;多于&nbsp;<input type=radio value=less name="loginR">&nbsp;少于&nbsp;&nbsp;<input size=5 name="loginT" type=text> 次</td>
<td width=50% class=forumrowHighlight>消失天数:<input type=radio value=more name="vanishR" checked>&nbsp;多于&nbsp;<input type=radio value=less name="vanishR">&nbsp;少于&nbsp;&nbsp;<input size=5 name="vanishT" type=text> 天</td>
</tr>
<tr>
<td>注册天数:<input type=radio value=more name="regR" checked>&nbsp;多于&nbsp;<input type=radio value=less name="regR">&nbsp;少于&nbsp;&nbsp;<input size=5 name="regT" type=text> 天</td>
<td>发表帖数:<input type=radio value=more name="artcleR" checked>&nbsp;多于&nbsp;<input type=radio value=less name="artcleR">&nbsp;少于&nbsp;&nbsp;<input size=5 name="artcleT" type=text> 篇</td>
</tr>

<tr>
<td class=forumrowHighlight>用户金钱:<input type=radio value=more name="UWealth" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UWealth">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UWealth_value" type=text></td>
<td class=forumrowHighlight>用户经验:<input type=radio value=more name="UEP" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UEP">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UEP_value" type=text></td>
</tr>
<tr>
<td>用户魅力:<input type=radio value=more name="UCP" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UCP">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UCP_value" type=text></td>
<td>用户威望:<input type=radio value=more name="UPower" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UPower">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UPower_value" type=text></td>
</tr>
<tr>
<td class=forumrowHighlight>用户金币:<input type=radio value=more name="UMoney" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UMoney">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UMoney_value" type=text></td>
<td class=forumrowHighlight>用户点券:<input type=radio value=more name="UTicket" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UTicket">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UTicket_value" type=text></td>
</tr>
<tr>
<td class=forumrow><LI>以下条件请选取相应的VIP用户组进行查询</LI></td>
</tr>
<tr>
<td class=forumrowHighlight>Vip登记时间:<input type=radio value=more name="UVipStarTime" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UVipStarTime">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UVipStarTime_value" type=text></td>
<td class=forumrowHighlight>Vip截止时间:<input type=radio value=more name="UVipEndTime" checked>&nbsp;多于&nbsp;<input type=radio value=less name="UVipEndTime">&nbsp;少于&nbsp;&nbsp;<input size=5 name="UVipEndTime_value" type=text></td>
</tr>
</table>
</td></tr>
...
<tr><td colspan=5 class=forumrow align=center><B>请选择您需要进行的操作</B>：<input type="radio" name="useraction" value=1> 删除&nbsp;&nbsp;<input type="radio" name="useraction" value=3> 删除用户所有帖子&nbsp;&nbsp;<input type="radio" name="useraction" value=2 checked> 移动到用户组
...
Sub ToUser()
	...
	if not founderr then
		if request("useraction")=1 then
			...
			Response.write "<tr><td colspan=8 class=forumrow>删除用户（ "& AllUserName &" ）操作成功。</td></tr>"
		...
		elseif request("useraction")=3 then
			...
			Dvbbs.Execute(sql)
			response.write "<tr><td colspan=8 class=forumrow>删除成功，如果要完全删除帖子请到论坛回收站<BR>建议您到更新论坛数据中更新一下论坛数据，或者<a href=alldel.asp>返回</a></td></tr>"
		else
			response.write "<tr><td colspan=8 class=forumrow>错误的参数。</td></tr>"
		end if
	end if
...
<tr><td class=forumrow height=23 align=left colspan=6>
...
<tr>
<td width=20% class=forumrow>用户名</td>
<td width=80% class=forumrow colspan=5><input size=45 name="username" type=text value="<%=Server.HtmlEncode(rs("username"))%>" disabled></td>
</tr>
<tr>
...
<td width=20% class=forumrow>用户性别</td>
<td width=80% class=forumrow colspan=5>
女 <input type="radio" value="0" <%if rs("UserSex")=0 then%>checked<%end if%> name="sex">&nbsp;
男 <input type="radio" value="1" <%if rs("UserSex")=1 then%>checked<%end if%> name="sex">&nbsp;
</td>
</tr>
<tr>
<td width=20% class=forumrow>个人照片</td>
<td width=80% class=forumrow colspan=5><input size=45 name="UserPhoto" type=text value="<%If Trim(rs("UserPhoto"))<>"" Then Response.Write Server.HtmlEncode(rs("UserPhoto"))%>"></td>
</tr>
<tr>
...
<td width=80% class=forumrow colspan=5><input size=45 name="face" type=text value="<%If Trim(Rs("UserFace"))<>"" Then Response.Write Server.HtmlEncode(Split(rs("userface"),"|")(Ubound(Split(rs("userface"),"|"))))%>">&nbsp;宽度：<input size=3 name="width" type=text value="<%=rs("userwidth")%>">&nbsp;高度：<input size=3 name="height" type=text value="<%=rs("userheight")%>"></td>
</tr>
<tr>
...
<tr>
<td width=20% class=forumrow>金币</td>
<td width=80% class=forumrow colspan=5><input size=45 name="usermoney" type=text value="<%=rs("usermoney")%>"></td>
</tr>
<tr>
<td width=20% class=forumrow>点券</td>
<td width=80% class=forumrow colspan=5><input size=45 name="UserTicket" type=text value="<%=rs("UserTicket")%>"></td>
</tr>
<tr>
...
<tr>
<td width=20% class=forumrow>阳光会员</td>
<td width=80% class=forumrow colspan=5>
关闭 <input type="radio" value="0" <%if rs("IsChallenge")=0 then%>checked<%end if%> name="IsChallenge">&nbsp;
开启 <input type="radio" value="1" <%if rs("IsChallenge")=1 then%>checked<%end if%> name="IsChallenge">&nbsp;
&nbsp;&nbsp;
...
<tr>
<td width=20% class=forumrow>VIP用户登记时间</td>
<td width=80% class=forumrow colspan=5>
<INPUT TYPE="text" NAME="Vip_StarTime" value="<%=Rs("Vip_StarTime")%>">
</td>
</tr>
<tr>
<td width=20% class=forumrow>VIP用户到期时间</td>
<td width=80% class=forumrow colspan=5>
...
Sub SaveUserInfo()
	...
	If Dvbbs.StrLength(Request.Form("sign")) > 250 Then
		Response.Write "<tr><td colspan=8 class=forumrow>用户签名不能超过 250 个字符。</td></tr>"
		Founderr = True
	End If
	if not founderr then
	...
	If Rs.Eof And Rs.Bof Then
		Response.Write "<tr><td colspan=8 class=forumrow>所选用户组信息并不存在。</td></tr>"
		Founderr = True
	Else
	...
	if not founderr then
		...
		If DvApi_Enable Then
			Set DvApi_Obj = New DvApi
				...
				DvApi_Obj.SendHttpData
				If DvApi_Obj.Status = "1" Then
					response.write "<tr><td colspan=8 class=forumrow>"&DvApi_Obj.Message&"</td></tr>"
				End If
			Set DvApi_Obj = Nothing
		End If
		'-----------------------------------------------------------------
...
Sub UserPermission()
	...
	If Not Founderr Then
		...
		Response.Write "<tr><td colspan=8 class=forumrow height=25><a href=?action=UserBoardPermission&boardid=0&userid=" & Request("Userid") & ">编辑该用户在全局的权限</a>（前台短信、前台用户信息、帖子和权限管理、进入后台权限等）</td></tr>"
		Else
		Response.Write "<tr><td colspan=8 class=forumrow height=25><a href=?action=UserBoardPermission&boardid=0&userid=" & Request("Userid") & "><font color=red>编辑该用户在全局的权限</font></a>（前台短信、前台用户信息、帖子和权限管理、进入后台权限等）</td></tr>"
		End If
'----------------------boardinfo--------------------
...
Sub SaveUserPermission()
	...
	if founderr then
		...
		response.write "<tr><td colspan=8 class=forumrow><li>设置用户权限成功。"
