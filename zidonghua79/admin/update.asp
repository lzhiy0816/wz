...
今日帖
<input type="checkbox" name="u5" value="1" checked>
昨日帖
<input type="checkbox" name="u6" value="1">
总固顶
<input type="checkbox" name="u7" value="1">
最后注册
<BR><BR><input type="submit" name="Submit" value="更新论坛总数据"><BR><BR>这里将重新计算整个论坛的帖子主题和回复数，今日帖子，最后加入用户等，建议每隔一段时间运行一次。<hr size=1></td>
</tr>
<tr>
<td width="20%" class="forumrow">更新分版面数据</td>
<td width="80%" class="forumrow"><input type="submit" name="Submit" value="更新分版面数据"><BR><BR>这里将重新计算每个版面的帖子主题和回复数，今日帖子，最后回复信息等，建议每隔一段时间运行一次。<hr size=1>
</td>
</tr>
<tr>
<td width="20%" class="forumrow">更新论坛收藏夹</td>
<td width="80%" class="forumrow"><input type="submit" name="Submit" value="更新收藏夹"><BR><BR>这里将重新整理论坛的收藏夹，删除不存在用户的收藏记录，重新指向被移动的帖子收藏地址，删除已被删除的帖子收藏记录。
</td>
</tr>
<tr> 
...
Sub Updateall()
	'主题数,帖子数,用户数,今日贴,昨日贴,总固顶,最后注册
	Body = "<tr><td colspan=2 class=forumrow>更新总论坛数据成功。"
	Dim AllTopNum,PostNum,TopicNum,LastUser
	Dim TodayNum,UserNum, YesterdayNum,SqlStr,Sql_a
...
Sub Delallonline()
	...
	Dvbbs.Execute(Sql)
  Body = "<tr><td colspan=2 class=forumrow>清空在线用户数据成功，请 <a href=""ReloadForumCache.asp""><font color=""red"">更新服务器缓存</font> </a>。"
End Sub
