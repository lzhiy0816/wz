...
<tr>
<td class="forumRow" style="line-height : 18px ;">
...
交易金额：
多于
<input type=radio value="0" name="MoneySize" <%If MoneySize = 0 Then Response.Write "Checked"%>>
少于
<input type=radio value="1" name="MoneySize" <%If MoneySize = 1 Then Response.Write "Checked"%>>
<input type=text size=10 name="PayMoney" value="<%=PayMoney%>">
<input name="submit" value="提交查询" type=Submit>
</td>
</tr>
</FORM>
</table><br>
...
<%
	...
	If Rs.Eof And Rs.Bof Then
		Response.Write "<tr><td class=forumRow height=23 colspan=6>未找到相关的交易信息。</td></tr>"
	Else
...
%>
<tr align=center>
<td class="forumRow" height=23><a href="../dispuser.asp?name=<%=Server.HtmlEncode(SQL(1,i))%>" target=_blank><%=Server.HtmlEncode(SQL(1,i))%></a></td>
<td class="forumRow"><%=SQL(2,i)%></td>
<td width="35" class="forumRow"><%=SQL(0,i)%></td>
...
<%
	...
	Rs.Close
	Set Rs=Nothing
	Response.Write "<tr><td class=forumRow height=23 colspan=6><B>本页交易金额</B>：<B>"&PageAmount&"</B> 元人民币，<B>本次查询交易总金额</B>：<B>"&AllAmount&"</B> 元人民币</td></tr>"
