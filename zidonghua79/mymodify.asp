...
Sub Userinfo()
	...
	If Ubound(iMagicFace) = 1 Then
		...
		Set mRs = Dvbbs.Plus_Execute("Select iMoney,iTicket From Dv_Plus_Tools_MagicFace Where MagicFace_s = " & Dvbbs.checkstr(iMagicFace(0)))
...
Sub update()
	...
	If tMagicFace = 1 And iMagicFace > 0 And Dvbbs.Forum_Setting(98)="1" Then
		...
		Else
			...
			If cCur(iMagicSetting(2)) > cCur(Dvbbs.UserSession.documentElement.selectSingleNode("userinfo/@userep").text) Then Response.redirect "showerr.asp?ErrCodes=<li>您的经验数没有达到使用魔法表情的标准，2秒后自动返回上一页面。&action=OtherErr&autoreload=1"
