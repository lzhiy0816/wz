...
'			Response.Write "parent.close_postform();alert('"&PostRetrunName&"');"'parent.close_postform();"
		...
		UserFlashGet=0'禁止使用 FlashGet避免缺少函数出错，原本可以包含FlashGet函数文件的，但这文件实在太垃圾了，一包含就有数据输出。 by老迷
		dv_ubb.PostType=1
		...
		Set Rs3=Dvbbs.Execute("select UserEmail,UserPost,UserSex,UserFace,UserWidth,UserHeight,UserIM,JoinDate,UserClass,userEP,UserPower,UserIsBest,UserTitle,UserGroupID,TitlePic  From Dv_User Where UserID="&Dvbbs.UserId)
